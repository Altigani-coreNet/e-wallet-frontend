        ...
        if (terminals.length === 0) {
            return (
                <div className="m-0 text-center py-10">
                    <div className="text-gray-500 fs-6">{t('admin.paymentGetway.noTerminalsFound')}</div>
                </div>
            );
        }
                    ...
                    <div className="timeline timeline-border-dashed">
                        <div className="timeline-item">
                            ...
                            <div className="timeline-content m-0">
                                <span className={`fs-8 fw-bolder text-${statusColor} text-uppercase`}>{t('admin.paymentGetway.terminalIdLabel')}</span>
                                <a href="#" className="fs-6 text-gray-800 fw-bold d-block text-hover-primary">
                                    {terminal.terminal_id || t('admin.paymentGetway.na')}
                                </a>
                                <span className="fw-semibold text-gray-500">{terminal.name || t('admin.paymentGetway.unnamed')}</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div className="card card-flush h-xl-100">
            <div className="card-header pt-7">
                <h3 className="card-title align-items-start flex-column">
                    <span className="card-label fw-bold text-gray-800">{t('admin.paymentGetway.terminalStatus')}</span>
                    <span className="text-gray-500 mt-1 fw-semibold fs-6">
                        {t('admin.paymentGetway.terminalsInUse', { count: loading ? '--' : terminalStatus.onlineCount || 0 })}
                    </span>
                </h3>
            </div>
            <div className="card-body pt-4 px-0" style={{ overflow: loading ? 'hidden' : 'visible' }}>
                ...
                <ul className="nav nav-pills nav-pills-custom item position-relative mx-9 mb-9" role="tablist">
                    <li className="nav-item col-4 mx-0 p-0" role="presentation">
                        ...
                        >
                            <span className="nav-text text-gray-800 fw-bold fs-6 mb-3">
                                {t('admin.paymentGetway.onlineTab', { count: loading ? '--' : terminalStatus.onlineCount || 0 })}
                            </span>
                    ...
                    <li className="nav-item col-4 mx-0 px-0" role="presentation">
                        ...
                        >
                            <span className="nav-text text-gray-800 fw-bold fs-6 mb-3">
                                {t('admin.paymentGetway.offlineTab', { count: loading ? '--' : terminalStatus.offlineCount || 0 })}
                            </span>
                            <span className="bullet-custom position-absolute z-index-2 bottom-0 w-100 h-4px bg-primary rounded"></span>
                        </a>
                    </li>
                    <li className="nav-item col-4 mx-0 px-0" role="presentation">
                        ...
                        >
                            <span className="nav-text text-gray-800 fw-bold fs-6 mb-3">
                                {t('admin.paymentGetway.testingTab', { count: loading ? '--' : terminalStatus.testingCount || 0 })}
                            </span>
                            <span className="bullet-custom position-absolute z-index-2 bottom-0 w-100 h-4px bg-primary rounded"></span>
                        </a>
                    </li>
