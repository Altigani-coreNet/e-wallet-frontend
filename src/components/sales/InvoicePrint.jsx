...
const InvoicePrint = () => {
    ...
    useEffect(() => {
        const fetchInvoice = async () => {
            try {
                setLoading(true);
                // Using public endpoint - no authentication required
                const fetchResponse = await fetch(`${POS_API_BASE}/invoice/${id}`);
                const result = await fetchResponse.json();
                
                console.log('Invoice API Response:', result);
                
                if (fetchResponse.ok && result.data) {
        ...
        <div className="invoice-container">
            ...
            <style>{`
                ...
                @media print {
                    ...
                    #kt_app_sidebar, #kt_app_header, #kt_app_toolbar, nav, aside { display: none !important; }
