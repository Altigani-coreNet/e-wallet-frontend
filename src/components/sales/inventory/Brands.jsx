...
export default function Brands() {
    ...
    useEffect(() => {
        ...
        setBreadcrumbs([
            ...
            { label: 'Brands', path: '/sales/brands' },
            { label: 'List Brands', path: '/sales/brands', active: true }
        ]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            queryClient.invalidateQueries({ queryKey: inventoryKeys.brands.all });
            await refetch();
        } catch (error) {
    ...
    const handleBulkDelete = useCallback(async () => {
        ...
        const result = await Swal.fire({
            title: 'Are you sure?',
            text: `You are about to delete ${selectedIds.length} brand(s). This action cannot be undone!`,
            icon: 'warning',
        ...
        if (result.isConfirmed) {
            try {
                ...
                if (response.success) {
                    await Swal.fire({
                        title: 'Deleted!',
                        text: 'Brands have been deleted successfully.',
                        icon: 'success',
                        timer: 2000,
                    ...
                    Swal.fire('Error!', response.error || 'Failed to delete brands.', 'error');
                }
            } catch (error) {
        ...
        <>
                ...
                <div className="card-body pt-0">
                    ...
                    {queryError && (
                        <div className="alert alert-danger alert-dismissible mb-4">
                            <strong>Error:</strong> {queryError?.response?.data?.message || queryError?.message || 'Failed to load brands'}
                            <button type="button" className="btn-close" onClick={() => queryClient.invalidateQueries({ queryKey: inventoryKeys.brands.list(queryParams) })}></button>
                        </div>
                    )}
                                    ...
                                    <th className="w-25px">
                                        <div className="form-check form-check-sm form-check-custom form-check-solid">
                                            <input
                                                ...
                                                type="checkbox"
                                                checked={selectedIds.length > 0 && selectedIds.length === brands.length}
                                                onChange={(e) => {
                                                    if (e.target.checked) {
                            ...
                            <form onSubmit={handleSubmit}>
                                <div className="modal-body">
                                    ...
                                    <div className="mb-3">
                                        ...
                                        <input
                                            key={editingBrand ? `edit-${editingBrand.id}` : 'new'}
                                            type="file"
                                            className="form-control"
                                            accept="image/*"
                                            onChange={(e) => {
                                        ...
                                        />
                                        {editingBrand && editingBrand.image && !formData.image && (
                                            <div className="mt-2">
                                                <small className="text-muted">Current image:</small>
                                                <img 
                                                    src={editingBrand.image} 
                                                    alt={editingBrand.name}
                                                    style={{ width: '60px', height: '60px', objectFit: 'cover', borderRadius: '4px', marginLeft: '10px' }}
                                                />
                                            </div>
                                        )}
                                        {formData.image && (
                                            <div className="mt-2">
                                                <small className="text-muted">Selected image: {formData.image.name}</small>
                                            </div>
                                        )}
                                    </div>
                                    <div className="mb-3">
            ...
            {showImportModal && (
                <ImportInventoryModal
                    ...
                    entityName="brand"
                    exportTemplate={exportBrandTemplate}
                    importPreview={importBrandsPreview}
                    importData={importBrands}
                />
            )}
        </>
    );
