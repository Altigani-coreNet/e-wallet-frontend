...
export default function Units() {
    ...
    useEffect(() => {
        ...
        setBreadcrumbs([
            ...
            { label: 'Units', path: '/sales/units' },
            { label: 'List Units', path: '/sales/units', active: true }
        ]);
    ...
    const handleSubmit = async (e) => {
        ...
        // Only send what the API expects in a clean shape
        const normalizedName = (formData.name?.en || '').trim();
        const payload = {
            name: { en: normalizedName },
            code: (formData.code || '').trim(),
            status: Number(formData.status) ? 1 : 0,
        };
        ...
        try {
            ...
            queryClient.invalidateQueries({ queryKey: inventoryKeys.units.all });
            await refetch();
        } catch (error) {
            ...
            const errorMessage = error.response?.data?.message ||
                (validationErrors ? Object.values(validationErrors).flat().join(', ') : 'Failed to save unit');
    ...
    const handleBulkDelete = useCallback(async () => {
        ...
        const result = await Swal.fire({
            title: 'Are you sure?',
            text: `You are about to delete ${selectedIds.length} unit(s). This action cannot be undone!`,
            icon: 'warning',
        ...
        if (result.isConfirmed) {
            try {
                const response = await bulkDeleteUnits(selectedIds);
                
                if (response.success) {
                    await Swal.fire({
                        title: 'Deleted!',
                        text: 'Units have been deleted successfully.',
                        icon: 'success',
                        timer: 2000,
                    ...
                    Swal.fire('Error!', response.error || 'Failed to delete units.', 'error');
                }
            } catch (error) {
    ...
    const handleEdit = (unit) => {
        ...
        setFormData({
            name: {
                en: (unit?.name && typeof unit.name === 'object' ? unit.name.en : unit.name) || ''
            },
            code: unit.code || '',
    ...
    const handleCloseModal = () => {
        ...
        setEditingUnit(null);
        setFormErrors([]);
        setFormData({ name: { en: '' }, code: '', status: 1 });
    };
        ...
        <>
                ...
                <div className="card-body pt-0">
                    ...
                    {queryError && (
                        <div className="alert alert-danger alert-dismissible mb-4">
                            <strong>Error:</strong> {queryError?.response?.data?.message || queryError?.message || 'Failed to load units'}
                            <button type="button" className="btn-close" onClick={() => queryClient.invalidateQueries({ queryKey: inventoryKeys.units.list(queryParams) })}></button>
                        </div>
                    )}
                                    ...
                                    <th className="w-25px">
                                        <div className="form-check form-check-sm form-check-custom form-check-solid">
                                            <input
                                                ...
                                                type="checkbox"
                                                checked={selectedIds.length > 0 && selectedIds.length === units.length}
                                                onChange={(e) => {
                                                    if (e.target.checked) {
                    ...
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h5 className="modal-title">{editingUnit ? 'Edit Unit' : 'Add New Unit'}</h5>
                            ...
                            <form onSubmit={handleSubmit}>
                                <div className="modal-body">
                                    ...
                                    <div className="mb-3">
                                        <label className="form-label required">Unit Code</label>
                                        <input
                                            type="text"
                                            className="form-control"
                                            placeholder="Enter unit code (e.g., KG, L, PCS)"
                                            value={formData.code}
                                            onChange={(e) => setFormData({ ...formData, code: (e.target.value || '').toUpperCase() })}
                                            required
                                            maxLength="10"
                                        />
                                        <small className="form-text text-muted">Unique code for this unit (e.g., KG, L, PCS, M)</small>
                                    </div>
                                    <div className="mb-3">
            ...
            {showImportModal && (
                <ImportInventoryModal
                    ...
                    entityName="unit"
                    exportTemplate={exportUnitTemplate}
                    importPreview={importUnitsPreview}
