...
import { exportTags, useAdminTags } from '../../../services/adminTagsService';
...
import TagFiltersPanel from './TagFiltersPanel';
import { downloadCSV } from '../../../utils/export';
import { formatDateTime, getTranslatedText } from '../../../utils/helpers';

const extractTagsFromPayload = (payload) => {
...
const isSuccessfulTagsResponse = (payload) => {
    if (!payload) {
        return false;
    }
    ...
    return Array.isArray(extractTagsFromPayload(payload));
};
...
const AdminTagsIndex = () => {
    ...
    const handleExport = useCallback(async () => {
        try {
            const response = await exportTags({ ...appliedFilters });
            const success = response?.success ?? response?.status;

            const exportPayload = response?.data;
            const exportRows = exportPayload?.data || exportPayload?.tags || [];
            if (success && exportRows.length > 0) {
                ...
                downloadCSV(transformedRows, exportPayload?.filename || 'tags_export.csv');
                toast.success('Tags export ready');
            } else {
                toast.info('No tags to export');
            }
        } catch (error) {
    ...
    const {
        ...
        isFetching,
        error: tagsError,
    } = useAdminTags(tagParams);

    useEffect(() => {
        if (!tagsData || !isSuccessfulTagsResponse(tagsData)) {
            return;
        }

        setPagination(prev => {
            const tagsList = extractTagsFromPayload(tagsData) ?? [];
            const paginationMeta = extractPaginationFromPayload(tagsData, tagsList.length, prev);
            const next = {
                ...prev,
                ...paginationMeta,
            };
    ...
    useEffect(() => {
        if (!tagsData) return;
        if (!isSuccessfulTagsResponse(tagsData)) {
            const message = getPayloadMessage(tagsData) || 'Failed to load tags';
            toast.error(message);
        }
    }, [tagsData]);
    ...
    useEffect(() => {
        if (!tagsError) return;
        const message = tagsError?.response?.data?.message || tagsError.message || 'Failed to load tags';
        toast.error(message);
    }, [tagsError]);

    const tags = useMemo(() => {
        if (!tagsData || !isSuccessfulTagsResponse(tagsData)) return [];
        return extractTagsFromPayload(tagsData) ?? [];
    }, [tagsData]);

    const tagShopIds = useMemo(() => {
        if (!tags.length) return [];
        const ids = new Set();
        tags.forEach((tag) => {
            const shopId = tag?.shop_id;
    ...
    const merchantLookups = useMerchantCountryLookup(tagShopIds);
    ...
    const resolvedPagination = useMemo(() => {
        ...
        const tagsList = extractTagsFromPayload(tagsData) ?? [];
        return extractPaginationFromPayload(tagsData, tagsList.length, pagination);
    }, [tagsData, pagination]);
                        ...
                        <div className="card-title">
                            <div className="d-flex align-items-center position-relative">
                                ...
                                <input
                                    ...
                                    placeholder="Quick search: Tag name, slug..."
                        ...
                        <table className="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
                            ...
                            <tbody>
                                {tags.length > 0 ? tags.map((tag) => (
                                    <tr key={tag.id}>
                                        <td><span className="text-dark fw-bold">{tag.id}</span></td>
                                        <td>{renderMerchant(tag?.shop_id)}</td>
                                        <td><span className="text-dark fw-bold">{getTranslatedText(tag.name) || 'N/A'}</span></td>
                                        <td><span className="badge badge-light-primary">{tag.slug || 'N/A'}</span></td>
                                        <td>{renderCountry(tag?.shop_id)}</td>
                                        <td><span className="text-muted">{formatDateTime(tag.created_at) || 'N/A'}</span></td>
                                        <td className="text-end">
                                            <Link to={`/admin/sales/tags/${tag.id}`} className="btn btn-sm btn-light-primary">View</Link>
                                        </td>
                                    </tr>
                                )) : (
                                    <tr><td colSpan="7" className="text-center py-5"><div className="text-muted">No tags found</div></td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
