...
import { exportSales, useAdminSales } from '../../../services/adminSalesService';
...
import SaleFiltersPanel from './SaleFiltersPanel';
...
const AdminSalesIndex = () => {
    ...
    const handleExport = useCallback(async () => {
        try {
            const response = await exportSales({ ...appliedFilters });
            ...
            if (success && exportRows.length > 0) {
                downloadCSV(exportRows, exportPayload?.filename || 'sales_export.csv');
                toast.success('Sales export ready');
            } else {
    ...
    const {
        ...
        isFetching,
        error: salesError,
    } = useAdminSales(salesParams);

    useEffect(() => {
        if (!salesData || salesData.success === false || !salesData.data) {
            return;
        }

        const { current_page, per_page, total, last_page } = salesData.data || {};
    ...
    useEffect(() => {
        ...
        if (salesData.success === false) {
            const message = salesData?.message || salesData?.error || salesData?.data?.message || 'Failed to load sales';
            toast.error(message);
        }
    }, [salesData]);
    ...
    useEffect(() => {
        ...
        const message = salesError?.response?.data?.message || salesError.message || 'Failed to load sales';
        toast.error(message);
    }, [salesError]);

    const sales = useMemo(() => {
        if (!salesData || salesData.success === false) return [];
        const container = salesData?.data?.data || salesData?.data?.items || [];
        return Array.isArray(container) ? container : [];
    }, [salesData]);

    const saleShopIds = useMemo(() => {
    ...
    const merchantLookups = useMerchantCountryLookup(saleShopIds);
                            ...
                            <table className="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
                                <thead>
                                    <tr className="fw-bold text-muted">
                                        ...
                                        <th style={{ cursor: 'pointer' }} onClick={() => handleSort('payment_status')} className="user-select-none">
                                ...
                                <tbody>
                                    {Array.from({ length: 5 }).map((_, i) => (
                                        <tr key={i}>
                                            {[...Array(8)].map((_, j) => (<td key={j}><span className="placeholder col-7"></span></td>))}
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                <div className="card-body py-4" style={{ position: 'relative' }}>
                    ...
                    <div className="table-responsive">
                        <table className="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
                            <thead>
                                <tr className="fw-bold text-muted">
                                    <th>ID</th><th>Reference</th><th>Customer</th><th>Grand Total</th><th>Payment Status</th><th>Merchant</th><th>Country</th><th className="text-end">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {sales.length > 0 ? sales.map((sale) => (
                                    <tr key={sale.id}>
                                        <td><span className="text-dark fw-bold">{sale.id}</span></td>
                                        <td><span className="text-dark fw-bold">{sale.reference_no}</span></td>
                                        <td><span className="text-muted">{sale.customer?.name || 'Walk-in'}</span></td>
                                        <td><span className="badge badge-light-success">${parseFloat(sale.grand_total || 0).toFixed(2)}</span></td>
                                        <td>
                                            <span className={`badge ${formatPaymentStatus(sale.payment_status).includes('Paid') ? 'badge-light-success' : formatPaymentStatus(sale.payment_status).includes('Partial') ? 'badge-light-warning' : 'badge-light-danger'}`}>
                                                {formatPaymentStatus(sale.payment_status)}
                                            </span>
                                        </td>
                                        <td>{renderMerchant(sale?.shop_id)}</td>
                                        <td>{renderCountry(sale?.shop_id)}</td>
                                        <td className="text-end">
                                            <Link to={`/admin/sales/sales-list/${sale.id}`} className="btn btn-sm btn-light-primary">View</Link>
                                        </td>
                                    </tr>
                                )) : (
                                    <tr><td colSpan="8" className="text-center py-5"><div className="text-muted">No sales found</div></td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
